...
#define DEF_FILEDIR "files"
#define DEF_SIZE 16348 // 16kB
...
int get_rom_size (char *str) {
    ...
    if ((errno == ERANGE && (val == LONG_MAX || val == LONG_MIN)) || (errno != 0 && val == 0)) {
       perror("Wrong SPIFFS size");
       exit(EXIT_FAILURE);
    } 

    if (endptr == str) {
        fprintf(stderr, "Wring SPIFFS size - no digits were found\n");
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char **args) {
	if (argc > 3) {
        printf ("Usage: %s spiffsSizeinBytes filesBaseDir\n", args[0]);
       exit (EXIT_FAILURE);
    }
	const char* FILEDIR = argc < 3 ? DEF_FILEDIR : args [2];
	const int SIZE = argc < 2 ? DEF_SIZE : get_rom_size (args [1]);
	
	printf("Creating rom %s of size %d bytes from files in dir %s\n", ROMNAME, SIZE, FILEDIR);
    rom = fopen(ROMNAME,"w+");
    int i;
    for(i=0; i < SIZE; i++) {
        fputc(ROMERASE,rom);
